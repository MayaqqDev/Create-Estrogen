...
class CentrifugingCRVRecipe(recipe: RecipeHolder<CentrifugingRecipe>) : CRVRecipe<CentrifugingRecipe>(recipe) {
    ...
    override val inputs: List<CRVIngredient> = listOf(CRVIngredient.of(recipe.value().inputs.first().fluid))

    override val outputs: List<CRVIngredient> = listOf(CRVIngredient.of(recipe.value().result.fluid))

    override val catalysts: List<CRVIngredient> =
        listOf(CRVIngredient.of(CreateEstrogenBlocks.Centrifuge.value!!.asItem().defaultInstance))

    companion object : ViewerInfo<CentrifugingRecipe, CentrifugingCRVRecipe>(
        CentrifugingRecipe,
        { CentrifugingCRVRecipe(it as RecipeHolder<CentrifugingRecipe>) },
