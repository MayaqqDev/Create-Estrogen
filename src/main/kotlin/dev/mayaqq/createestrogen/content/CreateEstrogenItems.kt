...
object CreateEstrogenItems: Registrar<Item> by Registrar(MOD_ID, Registries.ITEM) {
    val UsedFilter = item("used_filter", ::Item)
    val IncompleteEstrogenPatch = item("incomplete_estrogen_patches", ::SequencedAssemblyItem) {
        properties {
            stacksTo(1)
        }
    }
    val IncompleteUwU = item("incomplete_uwu", ::SequencedAssemblyItem) {
        properties {
            stacksTo(1)
        }
    }
